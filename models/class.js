...
exports.get_classes = function (body, callback) {
    ...
    mongoClient.connect(settings.mongodb_host, { useNewUrlParser: true }, function(err, client) {
        ...
        mongodb.collection('class').aggregate([
            { 
                $match: {
                    "schoolId": schoolId,
                    "name" : {$regex:search}
...
exports.add_class = function (body, callback) {
    var schoolId = body.schoolId;
    var name = body.name;
    var type = body.type;
    ...
    mongoClient.connect(settings.mongodb_host, { useNewUrlParser: true }, function(err, client) {
        ...
        assert.equal(null, err);
        var mongodb = client.db(settings.mongodb_dbname);
        mongodb.collection('class', function (err, collection) {
            if (err) {
                return callback(err);
                client.close();
            }
            collection.find({'name': name}).toArray(function(err, items) {
                ...
                if(items.length == 1){
                    ...
                    collection.find().sort({id:-1}).limit(1).toArray(function(err, res) {
                        ...
                        var insert_data = {id: str_new_id, schoolId: schoolId, name: name, type: type, status: status, createdAt: createdAt, changedAt: changedAt, timestamp: str_timestamp}
...
exports.edit_class = function (body, callback) {
    var id = body.id;
    var schoolId = body.schoolId;
    var name = body.name;
    var type = body.type;
        ...
        mongodb.collection('class', function (err, collection) {
            ...
            collection.find({'name': name}).toArray(function(err, items) {
                ...
                if(items.length == 1 && items[0].id != id){
                    ...
                    var changedAt = moment().format('YYYY-MM-DD hh:mm:ss');
                    var key_query = { id: id};
                    var value_query = {$set: {schoolId: schoolId, name: name, type: type, status: status, changedAt: changedAt}};
...
exports.del_class = function(id, callback) {
    mongoClient.connect(settings.mongodb_host, { useNewUrlParser: true }, function(err, client) {
        if (err) {
            return callback(err);
            client.close();
        ...
        assert.equal(null, err);
        var mongodb = client.db(settings.mongodb_dbname);
        mongodb.collection('class', {safe: true}, function (err, collection) {
            if (err) {
                return callback(err);
                client.close();
            }
...
exports.get_class_list = function (body, callback) {
    var schoolId = body.schoolId;
    mongoClient.connect(settings.mongodb_host, { useNewUrlParser: true }, function(err, client) {
        if (err) {
            return callback(err);
        ...
        mongodb.collection('class', function (err, collection) {
            ...
            collection.find({schoolId : schoolId, status : 'enabled'}).toArray(function(err, items) {
                if (err) return callback(err);
